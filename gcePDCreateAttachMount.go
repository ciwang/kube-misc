...
const (
	testProjectID        = "google.com:ciwang-kubernetes"
	testProjectZone      = "us-central1-b"
	testInstance0Name    = "e2e-test-ciwang-minion-group-21nq"
	testInstance1Name    = "e2e-test-ciwang-minion-group-62zl"
	diskByIdPath         = "/dev/disk/by-id/"
	diskGooglePrefix     = "google-"
	diskScsiGooglePrefix = "scsi-0Google_PersistentDisk_"
	testFSType           = "ext4"
	globalMountPath      = "/var/lib/ciwang/plugins/kubernetes.io/gce-pd/mounts/"
	finalMountPath       = "/var/lib/ciwang/pods/volumes/kubernetes.io~gce-pd/"
)
...
func main() {
	...
	//Before bind unmount: should fail
	exclOpenErr := verifyExclusiveOpen(devPath, testInstance0Name, true)
	log.Printf("Before bind unmount: Expect error, got %q", exclOpenErr);
	...
	//After bind unmount: should still fail
	exclOpenErr = verifyExclusiveOpen(devPath, testInstance0Name, true)
	log.Printf("After bind unmount: Expect error, got %q", exclOpenErr);		
	...
	//After global unmount: should succeed
	exclOpenErr = verifyExclusiveOpen(devPath, testInstance0Name, false)
	log.Printf("After global unmount: Expect error is nil, got %q", exclOpenErr);
...
func verifyExclusiveOpen(filePath string, instanceName string, expectErr bool) (error) {
	remoteCommand := "/home/ciwang/exclOpen " + filePath
	_, cmdErr := executeRemoteGCloudCmd(remoteCommand, instanceName)
	if (cmdErr != nil && !expectErr) || (cmdErr == nil && expectErr) {
		log.Printf(
			"Failed verifying with O_EXCL on block device %q. Expect error=%t, got %q.",
			filePath,
			expectErr,
			cmdErr)
	}
